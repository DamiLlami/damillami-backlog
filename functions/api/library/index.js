...
export const onRequest = async ({ request, env }) => {
  ...
  if (request.method === 'PUT') {
    ...
    // The client sends baseVersion = the updatedAt it last saw. If the library on the
    // server has moved on since then (another device wrote in the meantime), we must NOT
    // blindly overwrite — that would silently drop the other device's changes. Instead we
    // return 409 Conflict plus the current library, and let the client merge + retry.
    // Old clients that don't send baseVersion keep the legacy blind-write behavior so a
    // staged rollout doesn't break them.
    if (typeof body.baseVersion === 'number' && typeof lib.updatedAt === 'number') {
      if (body.baseVersion < lib.updatedAt) {
        return json({
          conflict: true,
          library: lib,
