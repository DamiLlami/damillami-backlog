...
export const onRequest = async ({ request, env }) => {
  ...
  const igdbSlug = url.searchParams.get('igdbSlug'); // e.g., "apex-legends"
  if (!title && !igdbId && !igdbSlug) {
    return new Response(JSON.stringify({ error: 'missing title, igdbId, or igdbSlug param' }), {
      status: 400,
  ...
  try {
    ...
    if (igdbId) {
      ...
      // Slug fetch (IGDB URLs are slug-based: /games/apex-legends — no numeric ID exposed in URLs)
      // Sanitize: only allow letters, numbers, and dashes
      const safeSlug = String(igdbSlug).toLowerCase().replace(/[^a-z0-9\-]/g, '');
      if (!safeSlug) throw new Error('Invalid slug');
      body = `where slug = "${safeSlug}"; fields name, summary, genres, platforms, first_release_date, total_rating_count, cover.image_id, franchise.name, collection.name; limit 1;`;
    } else {
