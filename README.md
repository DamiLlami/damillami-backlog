...
#### TMDb — rich data, great posters

The Movie Database has excellent structured metadata and high-quality posters for both movies and TV. With **"Compare across sources"** on (Settings → General), the picker queries OMDb *and* TMDb together and shows results from both side by side, badged by source, with the best match starred.

1. Create a free account at [themoviedb.org](https://www.themoviedb.org/)
2. Request a key: **Profile → Settings → API → Create → Developer** (the free v3 API key)
3. In Cloudflare env vars: `TMDB_API_KEY`
4. Redeploy (env var changes only apply to new deployments)
5. In the app: either set **Settings → Movies & TV — Source → TMDb**, or leave it on OMDb and turn on **Compare across sources** to use both at once
